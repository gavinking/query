...
[[left-joins-entities]]
==== Left joins to named entities

A left join to a named entity has the syntax:

[source,sql]
----
[from] left join Y as y
----

where, as before, `Y` is the name of the entity, `y` is the label (alias) assigned to it, and `[from]` is a root entity or a join.
As before, the keyword `as` is optional.

A left join may specify a predicate:

[source,sql]
----
[from] left join Y as y on [predicate]
----

where `[predicate]` is a predicate, as defined later in <<predicates>>.

A left join to a named entity produces a result list with:

- every structure belonging to the result list of the identical regular join,footnote:[That is, the same join with the `left` keyword deleted.] as defined above in <<joins-entities>>, and
- every structure _r_ of the result list of the operation on which the join acts for which there is no structure _r'_ in the result list of the identical regular join which contains every labeled element of _r_.

[[right-joins-entities]]
==== Right joins to named entities

A right join to a named entity has the syntax:

[source,sql]
----
[from] right join Y as y
----

where, as before, `Y` is the name of the entity, `y` is the label (alias) assigned to it, and `[from]` is a root entity or a join.
As usual, the keyword `as` is optional.

A right join may specify a predicate:

[source,sql]
----
[from] right join Y as y on [predicate]
----

where `[predicate]` is a predicate, as defined later in <<predicates>>.

A right join to a named entity produces a result list with:

- every structure belonging to the result list of the identical regular join, as defined above in <<joins-entities>>, and
- for each record _y_ of entity type `Y` which does not already occur with label `y` in the result list of the identical regular join, a structure containing _y_ labeled by the alias `y`.

[[full-joins-entities]]
==== Full joins to named entities

A full join to a named entity has the syntax:

[source,sql]
----
[from] full join Y as y
----

where, as before, `Y` is the name of the entity, `y` is the label (alias) assigned to it, and `[from]` is a root entity or a join.
As usual, the keyword `as` is optional.

A full join may specify a predicate:

[source,sql]
----
[from] full join Y as y on [predicate]
----

where `[predicate]` is a predicate, as defined later in <<predicates>>.

A full join to a named entity produces a result list with:

- every structure belonging to the result list of the identical regular join, as defined above in <<joins-entities>>,
- every structure _r_ of the result list of the operation on which the join acts for which there is no structure _r'_ in the result list of the identical regular join which contains every labeled element of _r_, and
- for each record _y_ of entity type `Y` which does not already occur with label `y` in the result list of the identical regular join, a structure containing _y_ labeled by the alias `y`.

That is, the result list of a full join contains every structure belonging to the result list of an identical left join, and every structure belonging to the result list of an identical right join.
...
[[left-joins-nested]]
==== Left joins to nested entities or collections
...
NOTE: This specification does not require support for right or full outer joins to nested entities or collections.
Some implementations of Jakarta Query might support such outer joins, but we do not define the semantics here.
...
Each value expression in the `order by` list must also occur in the projection list, as specified below in <<ordering-expressions>>.
...
[NOTE]
====
This specification defines the behavior of subqueries occurring:

- within <<restriction,restrictions>> in the `where` clause or `having` clause, or
- within <<select-clause,projections>>.
