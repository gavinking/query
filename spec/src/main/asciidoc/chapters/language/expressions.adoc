...
If any element of a path expression evaluates to a null value, the whole path expression evaluates to a null value.footnote:[However, the behavior of the `is null` operator, `coalesce()` function, and `nullif()` function is undefined when applied to a path with a null subpath, according to <<null-comparisons>> and <<function-application>>.]
...
NOTE: According to the rules given above in <<path-expressions>>, when a subpath of a path expression evaluates to null, the whole path expression is considered to evaluate to null.
However, this specification does not define the behavior of the `coalesce()` or `nullif()` function when applied to a path expression with a null subpath.
For backward compatibility with older releases of the Jakarta Persistence Query Language, an implementation of Jakarta Query is permitted to treat any conditional expression involving application of the `coalesce()` or `nullif()` function to a path with a null subpath as unsatisfied, contrary to the specification given here.
...
NOTE: According to the rules given above in <<path-expressions>>, when a subpath of a path expression evaluates to null, the whole path expression is considered to evaluate to null.
However, this specification does not define the behavior of a null comparison expression applied to a path expression with a null subpath.
For backward compatibility with older releases of the Jakarta Persistence Query Language, an implementation of Jakarta Query is permitted to treat such a null comparison as unsatisfied, contrary to the specification given here.
