...
An implementation of Jakarta Query is not required to allow the use of the inequality operators `<`, `>`, `<=`, and `>=` with time of day types like `LocalTime`.footnote:[Time of day is topologically a circle, and does not have a semantically meaningful total order.]
