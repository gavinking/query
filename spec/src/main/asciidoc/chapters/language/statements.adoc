...
- if the keyword `#left#` occurs in the `#join_spec#`, then the join is interpreted as a left (outer) join, as specified by <<left-joins-entities>> and <<left-joins-nested>>, or
- if the keyword `#right#` occurs in the `#join_spec#`, then the join is interpreted as a right (outer) join, as specified by <<right-joins-entities>>, or
...
In the persistence language, an `orderby_expression` may be almost any sort of atomic-valued expression (but not a parameter), as specified by the grammar of the persistence language.
