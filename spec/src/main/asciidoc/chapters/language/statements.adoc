...
In the common language, the `from` clause, with syntax given by `from_clause`, specifies an _entity name_ identifying the root entity of the query, as defined in <<root-entity>>.
...
NOTE: The common language does not have joins, and so the `from` clause is always trivial.

In the persistence language, the `from` clause might enumerate multiple steps of the basic operation pipeline.
The `#from_clause#` of the persistence language grammar allows:
...
Since a `from` clause may introduce multiple distinct labels, paths in the persistence language may begin with the label of an entity.
...
In the persistence language, an `#identification_variable_declaration#` might include a list of:
...
- if the keyword `#left#` occurs in the `#join_spec#`, then the join is interpreted as a left (outer) join, as specified by <<left-joins-entities>> and <<left-joins-nested>>,
- if the keyword `#right#` occurs in the `#join_spec#`, then the join is interpreted as a right (outer) join, as specified by <<right-joins-entities>>,
- if the keyword `#full#` occurs in the `#join_spec#`, then the join is interpreted as a full (outer) join, as specified by <<full-joins-entities>>, or
- otherwise, the join is interpreted as a regular (inner) join.
...
The `#group#` clause in the persistence language, with syntax given by `#groupby_clause#`, specifies a list of grouping items, which are collectively interpreted as an aggregation, as specified in <<aggregation>>.
...
The `#having#` clause in the persistence language, with syntax given by `#having_clause#`, specifies a restriction on the result list of an aggregation, as specified by <<restriction>>.
...
In the common language, the `select` clause, with syntax given by `select_clause`, specifies one or more path expressions with syntax `simple_path_expression`, which are collectively interpreted as a projection with automatically assigned aliases, as specified in <<projection>>.

Alternatively, the `select` clause may contain either:

- a single `count(this)` <<aggregate-functions,aggregate function application>>, or
...
In the persistence language, the `select` clause is much more flexible.
...
- optionally, in the persistence language, `#nulls first#` or `#nulls last#`.

In the common language, an `orderby_expression` must be a <<path-expressions,path expression>> or <<identifier-version-expressions,identifier expression>>.
In the persistence language, an `orderby_expression` may be almost any sort of atomic-valued expression (but not a parameter), as specified by the grammar of the persistence language and in <<ordering-expressions>>.
...
NOTE: These operations are part of the persistence language, and so support for union, intersection, and complementation is not required for an implementation of the Jakarta Common Query Language.
...
The syntax of `#union#`, `#union all#`, `#except#`, and `#except all#` is given by the rule `#union#` of the persistence language grammar.
...
The syntax of `#intersect#` and `#intersect all#` is given by the rule `#intersection#` of the persistence language grammar.
The operands must produce result lists of the same type.
...
In the common language, a `select` statement may contain one of more of the following clauses:
...
In the persistence language, a `select` statement may contain, in addition, a `#group#` and/or a `#having#` clause, with syntax specified by `#select_query#`.
Each `#select_query#` is a pipeline as defined in <<algebra>>.
A `#select_statement#` in the persistence language grammar may contain union, intersection, and complement operations, as defined in <<union-intersect-except>>.
