...
- <<function-application,function application>> and <<aggregate-functions,aggregate function application>>,
- <<map-key-value,map key and value expressions>> and <<identifier-version-expressions,entity identifier and version expressions>>,
...
- <<subquery-expressions,subqueries>>.

[[literal-expressions]]
==== Literal expressions

The syntax for literal expressions is given by the rules `string_literal`, `numeric_literal`, `#datetime_literal#`, and in the previous section titled <<Lexical structure>>.
...
In the persistence language:

- arbitrary-precision integer literals (with suffix `BI`) are assigned the type `java.math.BigInteger`,
- arbitrary-precision decimal literals (with suffix `BD`) are assigned the type `java.math.BigDecimal`,
- datetime literals (with prefix `DATETIME`) are assigned the type `java.time.LocalDateTime`,
- date literals (with prefix `DATE`) are assigned the type `java.time.LocalDate`, and
- time literals (with prefix `TIME`) are assigned the type `java.time.LocalTime`.
...
- the right operand of an `=` or `<>` equality comparison,
- an `in_item` element of an `in_item_list` occurring as the right operand of the <<in-expressions,`in` operator>>, or
- the `new_value` of an `update_item` in a `set` assignment.
...
In an implementation for Java, the type assigned must be a Java `enum` type, and the identifier must be either:

- the qualified name of an enumerated value of the `enum` type, including the fully qualified Java enum class name, or
- when the left operand of the comparison expression or `update_item` is assigned the enum type, just the unqualified name of the enumerated value.

For example, `day <> java.time.DayOfWeek.MONDAY` and `day <> MONDAY` are both legal comparison expressions.
...
In the persistence language, an _entity type literal_, with syntax given by `#entity_type_literal#` is just the name of an entity type, in the sense given in <<entities-and-embeddables>>.
...
- `simple_path_expression` in the common language, and
- `#single_valued_path_expression#`, `#atomic_valued_path_expression#`, `#embeddable_valued_path_expression#`, `#entity_valued_path_expression#`, `#collection_valued_path_expression#`, or `#joinable_path_expression#` in the persistence language.
...
However, the behavior of the `is null` operator, `coalesce()` function, or `nullif()` function is undefined when applied to a path with a null subpath.

[NOTE]
====
According to this definition, Jakarta Query treats path expressions as a sort of left outer join.
But for backward compatibility with older releases of the Jakarta Persistence Query Language, and contrary to the specification given here, an implementation of Jakarta Query is permitted to treat a path expression as if it were a sort of inner join.
That is, an expression like `book.publisher.name is null` might be unsatisfied when the subpath `book.publisher` is null.
====

NOTE: An implementation of the common language is not required to support path expressions which involve multiple entities. For example, if `Book` and `Publisher` are different entity types, then the path expression `book.publisher.name` is not part of the common language.footnote:[Path expressions involving multiple entities are called _implicit joins_ by the Jakarta Persistence specification.]
...
A _function application_ is the name of a function recognized by the Jakarta Query implementation, followed by a parenthesized list of argument expressions, with syntax given by:

- `function_expression` in the common language, or
- `#functions_returning_strings#`, `#functions_returning_datetime#`, and `#functions_returning_numerics#` in the grammar of the persistence language.

This specification defines the standard functions listed in the table below.

NOTE: Functions highlighted in yellow belong to the persistence language and are not required for an implementation of the common language.
...
| `#least#` | 2 or more | Any numeric type, any date or time type, or string | Same as arguments | Evaluates to the smallest argument, according to the <<natural-order,natural order>> of the type
| `#greatest#` | 2 or more | Any numeric type, any date or time type, or string | Same as arguments | Evaluates to the largest argument, according to the <<natural-order,natural order>> of the type
|===

When any argument expression of any function application evaluates to a null value, the whole function application evaluates to null.
...
In the persistence language, the `#extract()#` function accepts an expression assigned a date, time, or datetime type, along with an identifier -- a `#datetime_field#` or `#datetime_part#` -- indicating a specific part of the date, time, or datetime to extract, and evaluates to the specified part of the value to which its argument expression evaluates.
...
If the first argument of `#extract()#` is a `#datetime_field#` identifier, the function application is assigned the type `Integer`.
If the first argument of `#extract()#` is `#DATE#`, the function application is assigned the type `LocalDate`. If the first argument of `#extract()#` is `#TIME#`, the function application is assigned the type `LocalTime`.
...
This specification does not, therefore, require support for `#function()#`, not even in an implementation of the persistence language.
...
In the persistence language, a _map key expression_, _map value expression_, or _map entry expression_ is an application of the special `#key()#` function, special `#value()#` function, or special `#entry()#` function, respectively.

The `#entry()#` function may only occur in the <<select-clause,select clause>>, as specified by the grammar of the persistence language.
...
The persistence language provides three special functions for working with subtype polymorphism.

[[treat]]
===== Treated path expressions

A _treated path expression_ is an application of the special `#treat()#` function, with syntax given by `#treated_entity_path_expression#` and `#treated_joinable_path_expression#`.
...
A _coercion expression_ is an application of the special `#cast()#` function, with syntax given by the last alternatives of `#functions_returning_numerics#` and `#functions_returning_strings#`.
...
As suggested by the grammar for `#functions_returning_numerics#` and `#functions_returning_strings#`, implementations of the persistence language are strongly encouraged to support at least:
...
An _entity type expression_ is an application of the special `#type()#` function, with syntax given by `#type_discriminator#`.
...
An _aggregate function application_ may only occur in the `select` or `having` clause of a query involving <<aggregation,aggregation>>.
...
An aggregate function application is evaluated in the context of such a nested list.
...
In the common language, the only allowed aggregate function application is the expression `count(this)`, as specified below in <<select-clause>>.

In the persistence language, the syntax for aggregate functions is given by `#aggregate_expression#`.
An aggregate function application may specify the keyword `#distinct#`, in which case <<distinct,duplicate elimination>> is applied to the list of values produced by evaluating the argument expression over all elements of the nested list before counting or summing the values.footnote:[Use of `min(distinct ...)` or `max(distinct ...)` is allowed but redundant.]

[[case-expressions]]
==== Case expressions

In the persistence language, a _case expression_ has syntax given by `#general_case_expression#` or `#simple_case_expression#`.
...
In the grammar of the persistence language, a _constructor application_ is the name of a type belonging to the host programming language, followed by a parenthesized list of argument expressions, with syntax given by `#constructor_expression#`.

Constructor application can only occur as a `#select_expression#` in a projection.

In an implementation for Java, constructor application is interpreted as instantiation of a Java class or record type.
This specification does not require support for constructor application, not even in an implementation of the persistence language.
...
In the persistence language, a `#subquery#` may occur as a `scalar_expression` in many places where an expression of atomic type is legal.
...
The type assigned to a <<operator-expressions,binary numeric operator expression>> depends on the numeric types of its operand expressions, which need not be identical.
Implementations are required to tolerate numeric operator expressions involving operands assigned distinct numeric types.

NOTE: The behavior of numeric type promotion varies according to the database and client programming language.

In an implementation for Java, numeric type promotion is defined by the following rules:
...
- <<quantifiers,quantified conditional expressions>>,
- <<logical-operators,logical operator expressions>>, and
- boolean <<value-expressions,value expressions>>.
...
Every value expression of type `boolean` or `Boolean` is a legal conditional expression, satisfied when the value expression evaluates to `true`.
...
In the common language, an `in` expression, with syntax given by `in_expression`, must have:
...
In the persistence language, the leftmost operand must be a path expression assigned an atomic type or an <<entity-type-expressions,entity type expression>>, and there must be exactly one of the following elements:
...
- Its right operand must be a pattern given as a literal string, with syntax `literal_pattern`, or, in the persistence language, a <<parameters,parameter>>.
...
The operands of an equality or inequality operator must have the same type, unless eligible for <<numeric-promotion,numeric type promotion>>, and it must be either:

- an <<atomic-values,atomic type>> considered _comparable_ by the implementation of Jakarta Query,
- an entity or embeddable type, or
...
- For an entity type, an `=` equality expression is satisfied if and only if the two expressions evaluate to identical records; an `<>` inequality expression is satisfied if and only if the two expressions evaluate to distinct records.footnote:[Of course, in making this determination, it is sufficient to compare identifiers.]
- For an embeddable type, an `=` equality expression is satisfied if and only if the two expressions evaluate to identical structures; an `<>` inequality expression is satisfied if and only if the two expressions evaluate to distinct structures.footnote:[In making this determination, it is necessary to compare every labeled element.]
...
Entity and embeddable types are not considered to have a natural order, and so the behavior of other inequality operators is not defined by this specification.

An implementation of Jakarta Query is not required to allow the use of the inequality operators `<`, `>`, `<=`, and `>=` with time of day types like `LocalTime`.footnote:[Time of day can be understood algebraically as the circle group, which has no semantically meaningful total order.]
...
In the persistence language, the right operand of an equality or inequality operator may be a subquery, as discussed in the <<quantifiers,next section>>.

[[quantifiers]]
==== Quantified conditional expressions

In the persistence language, there are two kinds of conditional expression involving quantification:
...
An _ordering criterion_, with syntax given by `orderby_expression`, contributes to determining the order of a result list, as specified in <<ordering>>.
Each ordering criterion must be either:

1. a <<path-expressions,path expression>> assigned an atomic type with a natural order, and which evaluates to a value being returned by the query, either as an item in the select list, or as a labeled element of an entity or embeddable returned by the query,
2. in the persistence language, the name of a label declared by a `#select_item#` and assigned an atomic type with a natural order -- that is, a reference to a `#result_variable#` of the query, or
3. a `scalar_expression` involving only path expressions and result variables permitted by items 1 and 2 above.

An ordering criterion must never involve a <<parameters,parameter>> of the query.

[WARNING]
====
A database might impose additional restrictions on ordering criteria.
====
