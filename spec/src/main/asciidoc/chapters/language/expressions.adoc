...
If any element of a path expression evaluates to a null value, the whole path expression evaluates to a null value.footnote:[However, the behavior of the `is null` operator is undefined, according to <<null-comparisons>>.]
