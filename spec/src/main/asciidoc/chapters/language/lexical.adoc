...
The following identifiers are _keywords_: `abs`, `all`, `and`, `any`, `as`, `asc`, `avg`, `between`, `both`, `by`, `case`, `ceiling`, `class`, `coalesce`, `concat`, `count`, `current_date`, `current_time`, `current_timestamp`, `delete`, `desc`, `distinct`, `else`, `empty`, `end`, `entry`, `escape`, `except`, `exists`, `exp`, `extract`, `false`, `fetch`, `first`, `floor`, `from`, `function`, `greatest`, `group`, `having`, `in`, `index`, `inner`, `intersect`, `is`, `join`, `key`, `last`, `leading`, `least`, `left`, `length`, `like`, `local`, `ln`, `locate`, `lower`, `max`, `member`, `min`, `mod`, `new`, `not`, `null`, `nulls`, `nullif`, `object`, `of`, `on`, `or`, `order`, `outer`, `position`, `power`, `replace`, `right`, `round`, `select`, `set`, `sign`, `size`, `some`, `sqrt`, `substring`, `sum`, `then`, `trailing`, `treat`, `trim`, `true`, `type`, `union`, `update`, `upper`, `value`, `when`, `where`.footnote:[Jakarta Persistence reserves the following additional keywords for
...
In the common language, numeric literals come in two flavors:
...
In the persistence language, arbitrary-precision numeric literals are also provided:
...
=== Scope of identification variables

A reference to an identification variable (alias) may occur:

- in any <<path-expressions,expression>> occurring after its declaration, but directly within the query which declares the identification variable,
- in any expression occurring directly in the `select` clause of the query which declares the identification variable, or
- in a direct or indirect subquery of the query which declares the identification variable.

A reference to an identification variable declared by a subquery may not occur in a containing query of the subquery.